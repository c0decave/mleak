...
function detectServerHeaders(headers) {
  const out = [];
  // Null-prototype maps: family / receivedHits keys are constructed from
  // hardcoded family names (or `${family}:${host}` template-literals,
  // never the bare attacker host), so today this can't be poisoned —
  // but the design no longer holds that promise structurally. Keeping
  // both maps null-proto removes the prototype-lookup hazard for any
  // future refactor that exposes the bare host as the key.
  const seenFamilies = Object.create(null);
  ...
  const receivedHits = Object.create(null);
