...
// Null-prototype dictionary: lookups for attacker-controlled header names
// must not inherit `constructor` / `toString` / `__proto__` from
// Object.prototype. Without this guard, a mail header literally called
// `constructor` would make titleCaseHeader() return the Object function
// (truthy, non-string) and pollute the surfaced header_order summary.
const CANONICAL = Object.freeze(Object.assign(Object.create(null), {
...
// Null-prototype: `family` is set by claimedFamily() to one of a small
// hardcoded list today, but keeping the lookup table prototype-clean
// means a future regression that lets a header value flow into family
// can't surface an inherited Object method as the "expected" sequence.
const EXPECTED_PREFIXES = Object.freeze(Object.assign(Object.create(null), {
  thunderbird: [
    ["Date", "From", "To"],
    ["From", "Date", "Subject"],
    ["Message-ID", "Date"],
...
function headerOrder(headers) {
  const out = [];
  const seen = new Set();
  for (const key of Object.keys(headers || {}).slice(0, 256)) {
