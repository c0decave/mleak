...
const { getHeader, finding, CONF, sanitiseValue } = globalThis.OSINTUtil;
...
function allContentTypes(part, depth, acc) {
  ...
  // sanitiseValue() — same path bypass as mime_boundary.js (parts tree
  // read directly, bypassing _cap()). 0.6.16 hardening.
  if (part.contentType) acc.push(sanitiseValue(part.contentType));
  if (Array.isArray(part.parts)) {
    for (const sub of part.parts.slice(0, 64)) {
      if (acc.length >= 256) break;
      allContentTypes(sub, depth + 1, acc);
    }
  }
  return acc;
}
...
function detectCrypto(message) {
  ...
  if (enigmailVer) {
    out.push(finding("crypto", "enigmail_version",
      ...
      leaks: { family: "Enigmail", version: shortValue(enigmailVer),
  ...
  // Null-prototype lookup: the `protocol=` parameter is attacker-controlled
  // (any unquoted token to ;/" /whitespace). Without this guard, a value of
  // `__proto__`/`constructor`/`toString` would inherit Object.prototype's
  // member and surface a function or {} into the summary.
  const PROTO_LABEL = Object.assign(Object.create(null), {
  ...
  if (autocrypt) {
    ...
    out.push(finding("crypto", "autocrypt", "Autocrypt", CONF.HIGH, {
      leaks: {
        addr: addrMatch ? shortValue(addrMatch[1]) : null,
        prefer_encrypt: prefMatch ? shortValue(prefMatch[1]) : null,
      },
  ...
  if (openpgp) {
    ...
    out.push(finding("crypto", "openpgp_hint", "OpenPGP", CONF.MEDIUM, {
      leaks: {
        keyserver_url: urlMatch ? shortValue(urlMatch[1]) : null,
        key_id:        idMatch  ? shortValue(idMatch[1])  : null,
      },
  ...
  if (pgpu) {
    out.push(finding("crypto", "pgp_universal",
                     `Symantec PGP Universal (${shortValue(pgpu)})`, CONF.HIGH, {
      leaks: { state: shortValue(pgpu), src: "X-PGP-Universal" },
