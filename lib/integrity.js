...
function signedHeaderList(hTag) {
  return String(hTag || "").split(":")
...
function detectIntegrity(headers) {
  ...
  for (const sig of dkimSigs) {
    const flatSig = String(sig).replace(/\s+/g, " ");
    const hM = flatSig.match(/\bh\s*=\s*([^;]+)/i);
    if (!hM) continue;
    const hList = signedHeaderList(hM[1]);
    ...
    // hList is attacker-controlled (DKIM h= comes off-the-wire); use a
    // null-prototype object so crafted entries like __proto__ / constructor
    // can't shadow Object.prototype methods and skew the counts.
