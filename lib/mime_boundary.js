...
function collectContentTypes(part, depth, acc) {
  ...
  if (part.contentType && typeof part.contentType === "string") {
    // Sanitise on the way in. `part.contentType` is read straight off
    // the message tree (TB's getFull() output), bypassing the header
    // `_cap()` path. A crafted boundary like `boundary="Apple-Mail=_\r…"`
    // would otherwise surface the CR/LF in `summary.mime_boundaries[*]
    // .leaks.boundary`. Same bug class as the 0.6.16 body_html fix.
    acc.push(sanitiseValue(part.contentType.slice(0, 8192)));
  }
