...
const UA_RULES = [
  ...
  // CLI / automation
  // svnmailer is the Subversion commit-mailer script; very common on
  // open-source mailing-list archives (55k+ in the corpus's 5.2 M).
  // Versions are hyphen-separated: "svnmailer-1.0.9", "1.0.8-patched".
  { rx: /\bsvnmailer-([\w.\-]+)/, family: "svnmailer (Subversion mail script)" },
  // groups.io uses a literal "GROUPS.IO Web Poster" UA for posts via
  // its web UI; no version, but worth tagging as a known family so it
  // stops showing up in the unknown-UA bucket.
  { rx: /\bGROUPS\.IO Web Poster\b/, family: "groups.io (Web Poster)" },
  // git.sr.ht commit-mailer (sourcehut).
  { rx: /\bgit\.sr\.ht\b/, family: "git.sr.ht (sourcehut)" },
  // FastMail / MessagingEngine.com webmail. The trailing ajax-<hash> on
  // some lines is a build hash, not a version, so we don't capture it.
  { rx: /\bMessagingEngine\.com Webmail Interface(?:\s*-\s*ajax-[\w]+)?/,
    family: "FastMail (MessagingEngine.com)" },
