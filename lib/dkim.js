...
const DKIM_VENDOR_HINTS = [
  ...
  [/^google$|^20\d{6}$/, "Google / Gmail (dated selector)"],
  [/^zohomail$|^zoho\d*$/, "Zoho Mail"],
  [/^tucows$|^hostedemail/, "HostedEmail (Tucows/OpenSRS)"],
];
const MAX_DKIM_SIGNED_HEADERS = 64;

function shortValue(value, max = 200) {
...
function parseDkimSig(raw) {
  // Null prototype: the DKIM-Signature tag-name capture is `[a-zA-Z]{1,3}`,
  // which today can't form `__proto__`/`constructor` (those are >3 chars).
  // But anchoring this on a null prototype removes the prototype-lookup
  // class from the design entirely — if a future RFC extension widens the
  // tag-name character set, the parser can't be poisoned by a header.
...
function detectDKIM(headers) {
  ...
  for (const raw of sigs) {
    ...
    const leaks = { selector: shortValue(tags.s), domain: shortValue(tags.d),
                    algorithm: shortValue(tags.a), canon: shortValue(tags.c),
    ...
    const hTag = tags.h || "";
    leaks.signed_headers = hTag.split(":")
      .map(s => s.trim().slice(0, 80))
      .filter(Boolean)
      .slice(0, MAX_DKIM_SIGNED_HEADERS);

    out.push(finding("dkim", "signature",
                     `${shortValue(tags.d || "?")}/${shortValue(tags.s || "?")}`,
                     CONF.HIGH, { leaks }));
  }
  return out;
}
...
function detectAuthResults(headers) {
  ...
  for (const raw of ar) {
    // Null prototype: the keys we assign (dkim/spf/dmarc/arc/bimi/server)
    // are hardcoded, but the leaks object is later iterated by
    // mergeAuthVerdicts and the popup. Keep this side too on a null base
    // so the whole chain is prototype-clean.
    const leaks = Object.create(null);
    ...
    if (serverMatch) leaks.server = shortValue(serverMatch[1]);
